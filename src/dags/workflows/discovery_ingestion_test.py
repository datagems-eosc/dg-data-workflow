...
from common.extensions.callbacks import on_execute_callback, on_retry_callback, on_success_callback, \
    on_failure_callback, on_skipped_callback, on_success_cdd_ingestion_callback
...
from services.cdd_ingestion import DAG_ID, fetch_profile_path_builder, begin_ingestion_builder, \
...
@dag(DAG_ID + '_test', params=DAG_PARAMS, tags=["CDD_Ingest_test", ], dag_display_name=DAG_DISPLAY_NAME + '_test')
def discovery_ingestion():
    ...
    @task.sensor(on_execute_callback=on_execute_callback, on_retry_callback=on_retry_callback,
                 on_success_callback=on_success_cdd_ingestion_callback, on_failure_callback=on_failure_callback,
