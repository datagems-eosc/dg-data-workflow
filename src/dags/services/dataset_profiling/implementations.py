...
def trigger_profile_builder(auth_token: str, dag_context: Context, config: ProfilerConfig,
    ...
    if DataStoreKind(dag_context["params"]["data_store_kind"]) is DataStoreKind.FileSystem:
        ...
        instance_match = next((x for x in db_server_registry.instances if (
                    x.name == dag_context["params"]["database_name"] or (
        ...
        data_connectors.append(
            {
                "type": DataStoreKind(dag_context["params"]["data_store_kind"]).to_connector_type().value,
                "database_name": instance_match.name,
                "engine": instance_match.engine,
                "protocol": instance_match.protocol,
                "host": instance_match.host,
                "port": instance_match.port
            })
        ...
        "profile_specification":
            {
                ...
                "languages": dag_context["params"]["languages"] if dag_context["params"]["languages"] else [],
                "country": dag_context["params"]["countries"][0] if dag_context["params"]["countries"] else "",
...
def update_data_model_management_builder(access_token: str, dag_context: Context, dmm_config: DataModelManagementConfig,
    ...
    payload["ap"]["nodes"].extend(converted_profile["nodes"])
    payload["ap"]["edges"].extend(converted_profile["edges"])
